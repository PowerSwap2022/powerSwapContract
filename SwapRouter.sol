...
interface ISwapFactory {
    ...
    function setPairFee(address _token0, address _token1) external;
...
library SafeMath {
    ...
    function sub(uint x, uint y) internal view returns (uint z) {
...
library SwapLibrary {
    ...
    function pairFor(address factory, address tokenA, address tokenB) internal view returns (address pair) {
        ...
        pair = address(uint(keccak256(abi.encodePacked(
                ...
                hex'712d742a3f24b62fe45373d20bf3cd14be5a252c0ae67270574f8336e5186d7a' // init code hash
            ))));
    }
    ...
    function getAmountOut(uint amountIn, uint reserveIn, uint reserveOut, uint256 nu) internal view returns (uint amountOut) {
        ...
        uint amountInWithFee = amountIn.mul(nu);                    // 9975
    ...
    function getAmountIn(uint amountOut, uint reserveIn, uint reserveOut, uint256 nu) internal view returns (uint amountIn) {
        require(amountOut > 0, 'SwapLibrary: INSUFFICIENT_OUTPUT_AMOUNT');
        require(reserveIn > 0 && reserveOut > 0, 'SwapLibrary: INSUFFICIENT_LIQUIDITY');
        uint numerator = reserveIn.mul(amountOut).mul(10000);           
        uint denominator = reserveOut.sub(amountOut).mul(nu);           // 9975
        amountIn = (numerator / denominator).add(1);
    }

    // performs chained getAmountOut calculations on any number of pairs
    function getAmountsOut(address factory, uint amountIn, address[] memory path,uint256 nu) internal view returns (uint[] memory amounts) {
        require(path.length >= 2, 'SwapLibrary: INVALID_PATH');
        amounts = new uint[](path.length);
        amounts[0] = amountIn;
        for (uint i; i < path.length - 1; i++) {
            (uint reserveIn, uint reserveOut) = getReserves(factory, path[i], path[i + 1]);
            amounts[i + 1] = getAmountOut(amounts[i], reserveIn, reserveOut,nu);
        }
    }

    // performs chained getAmountIn calculations on any number of pairs
    function getAmountsIn(address factory, uint amountOut, address[] memory path,uint256 nu) internal view returns (uint[] memory amounts) {
        require(path.length >= 2, 'SwapLibrary: INVALID_PATH');
        amounts = new uint[](path.length);
        amounts[amounts.length - 1] = amountOut;
        for (uint i = path.length - 1; i > 0; i--) {
            (uint reserveIn, uint reserveOut) = getReserves(factory, path[i - 1], path[i]);
            amounts[i - 1] = getAmountIn(amounts[i], reserveIn, reserveOut,nu);
        }
    }
}
...
contract SwapRouter is ISwapRouter02, Ownable {
    ...
    function setTradingPool(address _tradingPool) public onlyManager() {
        tradingPool = _tradingPool;
    }

    function resetFeeToPair() public {
        uint256 _fee = ISwapFactory(factory).fee();
        fee = _fee;
    }
    ...
    function _swap(uint[] memory amounts, address[] memory path, address _to) internal virtual {
        for (uint i; i < path.length - 1; i++) {
            ...
            address pair = SwapLibrary.pairFor(factory, input, output);
             if(fee != ISwapPair(pair).fee()){
                ISwapFactory(factory).setPairFee(input,output);
            }
            ISwapPair(pair).swap(
    ...
    function swapExactTokensForTokens(
        ...
        amounts = SwapLibrary.getAmountsOut(factory, amountIn, path,feeDenominator-fee);
    ...
    function swapTokensForExactTokens(
        ...
        amounts = SwapLibrary.getAmountsIn(factory, amountOut, path,feeDenominator-fee);
        ...
        returns (uint[] memory amounts)
    {
        ...
        amounts = SwapLibrary.getAmountsOut(factory, msg.value, path,feeDenominator-fee);
        require(amounts[amounts.length - 1] >= amountOutMin, 'SwapRouter: INSUFFICIENT_OUTPUT_AMOUNT');
    ...
    function _swapSupportingFeeOnTransferTokens(address[] memory path, address _to) internal virtual {
        for (uint i; i < path.length - 1; i++) {
            ...
            amountOutput = SwapLibrary.getAmountOut(amountInput, reserveInput, reserveOutput,feeDenominator-fee);
            }
            if (tradingPool != address(0)) {
    ...
    {
        return SwapLibrary.getAmountOut(amountIn, reserveIn, reserveOut,feeDenominator-fee);
    }
    ...
    {
        return SwapLibrary.getAmountIn(amountOut, reserveIn, reserveOut,feeDenominator-fee);
    }
    ...
    function getAmountsOut(uint amountIn, address[] memory path)
        ...
        returns (uint[] memory amounts)
    {
        return SwapLibrary.getAmountsOut(factory, amountIn, path,feeDenominator-fee);
    }
    ...
    function getAmountsIn(uint amountOut, address[] memory path)
        ...
        returns (uint[] memory amounts)
    {
        return SwapLibrary.getAmountsIn(factory, amountOut, path,feeDenominator-fee);
    }
}