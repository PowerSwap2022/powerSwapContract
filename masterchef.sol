...
contract MasterChefV2 is Ownable, ReentrancyGuard {
    ...
    ///     reward rewards each pool gets.
    /// `accrewardPerShare` Accumulated rewards per share, times 1e12.
    ...
    ///     the reward rewards to all the PanrewardSwap products.
    ...
    /// @notice The only address can withdraw all the burn reward.
    ...
    /// @notice total reward rate = toBurn + toRegular + toSpecial
    uint256 public constant REWARD_RATE_TOTAL_PRECISION = 1e12;
    ...
    event UpdaterewardRate(uint256 burnRate, uint256 regularFarmRate, uint256 specialFarmRate);
    ...
    function setPowerToken(IERC20 _token) public onlyOwner{
        powerToken = _token;
    }
    ...
    /// @param _rewardPerBlock Whether the pool is regular or special. LP farms are always "regular". "Special" pools are
    ...
    function add(
        ...
        // stake reward token will cause staked token and reward token mixed up,
        // may cause staked tokens withdraw as reward token,never do it.
        require(_lpToken != powerToken, "reward token can't be added to farm pools");
        ...
        emit AddPool(lpToken.length.sub(1), _allocPoint, _lpToken, true);
    }
    ...
    function pendingreward(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 rewardReward = multiplier.mul(pool.rewardPerBlock);
        ...
        return boostedAmount.mul(accrewardPerShare).div(ACC_REWARD_PRECISION).sub(user.rewardDebt);
    }
    ...
    function updatePool(uint256 _pid) public returns (PoolInfo memory pool) {
        ...
        if (block.number > pool.lastRewardBlock) {
            ...
            if (lpSupply > 0 && pool.rewardPerBlock > 0) {
                uint256 multiplier = block.number.sub(pool.lastRewardBlock);
                uint256 rewardReward = multiplier.mul(pool.rewardPerBlock);
            ...
            poolInfo[_pid] = pool;
            emit UpdatePool(_pid, pool.lastRewardBlock, lpSupply, pool.accrewardPerShare);
        }
    }
    ...
    function deposit(uint256 _pid, uint256 _amount) external nonReentrant {
        ...
        user.rewardDebt = user.amount.mul(multiplier).div(BOOST_PRECISION).mul(pool.accrewardPerShare).div(
    ...
    function updateBoostMultiplier(
        ...
        user.rewardDebt = user.amount.mul(_newMultiplier).div(BOOST_PRECISION).mul(pool.accrewardPerShare).div(
            ACC_REWARD_PRECISION
        );
    ...
    function settlePendingreward(
        ...
        uint256 accreward = boostedAmount.mul(poolInfo[_pid].accrewardPerShare).div(ACC_REWARD_PRECISION);
