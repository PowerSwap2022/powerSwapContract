...
contract SwapERC20  {
    ...
    string public constant name = 'PowerSwap LPs';
    string public constant symbol = 'Power-LP';
...
library Math {
    function min(uint x, uint y) internal view returns (uint z) {
        z = x < y ? x : y;
    }
...
library UQ112x112 {
    ...
    function uqdiv(uint224 x, uint112 y) internal view returns (uint224 z) {
        z = x / uint224(y);
    }
}
...
contract SwapFactory {
    ...
    function setFeeToPair(uint256 _fee) external {
        require(msg.sender == feeToSetter, 'Swap: FORBIDDEN');
        require(_fee<= 30,'factory: invalid Value!');
        fee = _fee;
    }

    function setPairFee(address _token0, address _token1) public {
        address pair = getPair[_token0][_token1];
        require(pair != address(0),'factory: invalid address!');
        ISwapPair(pair).setFee(fee);
    }
